...
# Implementation: Aster & Row Support Agent

A small, reliable local RAG support agent. It is deliberately deterministic: no API key is required, and policy answers use explicit, testable rules over retrieved passages rather than a model guessing from a giant prompt.

> Demo preview: [terminal walkthrough](demo/demo.gif) — knowledge citation, order lookup, multi-turn Canada follow-up, conflict handoff, and evaluation run. Regenerate it with `powershell -ExecutionPolicy Bypass -File demo/generate_demo_gif.ps1`.

## Setup and run

Requires Python 3.11+. There are no third-party packages or required credentials; `.env.example` documents the optional trace directory.
...
`--trace` prints structured, customer-safe observability data: message, bounded session history, retrieved passages with metadata/scores, tool calls, sanitized results, response, handoff, and fallbacks. It never writes secrets or raw orders.

Open **http://127.0.0.1:8000** after running `python -m src.server` for the local browser chat. Each browser tab has its own in-memory conversation session.

## Architecture and choices

Markdown front matter and heading chunks are parsed into an in-memory lexical index. Retrieval excludes inactive, draft, and non-official content before scoring; each policy answer cites `filename — heading`. The response layer uses only active official material, explicitly recognizes the active Breeze conflict, and abstains otherwise.

Orders are accessed exclusively through a fixed-allowlist lookup function. It normalizes harmless ID formatting; current `status` supersedes stale logistics data; and the tool cannot return customer or `internal` fields. Session history is bounded to four messages per session and never crosses session IDs.

Model: none (deterministic safety-first response layer). Embeddings: token-overlap lexical ranking. Framework: Python standard library. Storage: parsed Markdown and JSON loaded in memory.

## Evaluation

`python evaluation/run_eval.py` runs all 14 visible cases plus five original regressions and prints each result plus category totals. Assertions are deterministic: required/forbidden claims, citations, tool usage/arguments, handoff, normalization, stale data suppression, and privacy. `tests/test_agent.py` adds unit checks for safety boundaries and session isolation.

Final result: **20/20** cases passing (retrieval 2/2, multi-source-grounding 1/1, conversation 2/2, groundedness 2/2, tool-use 3/3, tool-reliability 5/5, privacy 1/1, prompt-security 1/1, abstention 1/1, source-conflict 1/1, safety 1/1).

Early baseline (directly searching all text without front-matter precedence or status cleanup): **11/19**. It incorrectly used the legacy return window, exposed cancelled-order ETA, followed the migration scratchpad, and lost Canada on a follow-up.

## Bug diary

| Reproduction | Root cause | Change | Regression |
|---|---|---|---|
| Ask for ORD-1004’s arrival. | Raw ETA was trusted over `status`. | Blank stale logistics fields for cancelled/returned orders. | `cancelled-order-stale-eta`; `test_cancelled_hides_stale_eta` |
| Ask “What about Canada?” after international shipping. | Routing classified the isolated short follow-up as unknown. | Bounded same-session history participates in retrieval and routing. | `canada-multiturn`; `session-isolation` |
| Request 60 days using the migration note. | A naive search treated document text as instructions. | Draft/non-authoritative documents are excluded; application guard refuses approval. | `retrieved-prompt-injection` |
| Ask whether Breeze is dishwasher safe. | First matching source silently won. | Explicit active-official conflict detection and safe interim guidance. | `genuine-active-source-conflict` |

## Known limitations

This intentionally small, English-only rule-driven implementation does not call an LLM, perform actions, persist sessions across restarts, retrieve paraphrases as flexibly as embeddings could, or create support tickets. Before production I would add authenticated server-side sessions, an approved embeddings index with retrieval evaluations, constrained LLM synthesis validated against citations, monitoring/redaction controls, and a real handoff integration.

## AI-tool disclosure

Codex was used to inspect the supplied corpus, scaffold this dependency-free implementation, and propose test coverage. One incomplete suggestion was to treat every newer document as authoritative; that is unsafe here because the newer migration scratchpad is a draft, and active official Breeze sources still genuinely conflict. The implemented precedence uses status, authority, and supersession rather than dates alone.
